...
import { setHistoryPageState, setHistoryPageStateNext, setHistoryPageStatePrevious, setHistoryPageStateStart, setUserProducts } from "../sidebar/actions/productSlice";
...
const DataPagination = () => {
    ...
    const handleEnd = async (localPageState: string[], localPageIndex: number) => {
        dispatch(setHistoryPageState(localPageIndex))
        ...
        <div className="center">
            <Pagination style={{padding: '15px'}}>
                <ButtonGroup className="mb-2">
                    ...
                    <Button style={{border: '#FAF9F6 solid 1px'}} onClick={() => handleEnd(historyPageState, historyPageState.length-1)} disabled={userProducts.length < parseInt(productsPerPage) || noNextPage}>End<ChevronBarRight color="white" size={18}/></Button>
                </ButtonGroup>
            </Pagination>
        </div>
    );
